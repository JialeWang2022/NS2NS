...
def noisify(img,std,noise_type="gaussian",lam=30):
    if noise_type == "gaussian":
        img_noise = img + 0.6 * np.random.normal(loc=0.0, scale=1.0, size=img.shape)
...
def main():
    ...
    args.output_file = "{}/seg3600_0610gus_ran35_{}{}_ps{}_ns{}_lmdb".format(args.output_folder, args.noise_type,
                                                                  noise_param, args.patch_size, args.num_sim)

    if args.config_file is None:
        
        data_in_test = np.load("/mnt/4-fan.npy")
        img = data_in_test.reshape(args.data_num,128,128,1)
        data_noise = noisify(img, std = 1.0, noise_type=args.noise_type, lam=args.lam)
...
#         img_noise_sim = compute_sim_images(img_noise, patch_size=5, num_select=args.num_sim)
        ...
        img_noise_sim_1 = compute_sim_images(img_noise, patch_size=3, num_select=8)
        img_noise_sim_2 = compute_sim_images(img_noise, patch_size=5, num_select=8)
#         img_noise_sim_3 = compute_sim_images(img_noise, patch_size=7, num_select=5)

        img_noise_sim1_1 = img_noise_sim_1[random.randint(0,7),:,:,:].reshape(128,128)
        img_noise_sim2_1 = img_noise_sim_2[random.randint(0,7),:,:,:].reshape(128,128)
#         img_noise_sim3_1 = img_noise_sim_3[random.randint(0,4),:,:,:].reshape(128,128)
        
        img_noise_sim1_2 = img_noise_sim_1[random.randint(0,7),:,:,:].reshape(128,128)
        img_noise_sim2_2 = img_noise_sim_2[random.randint(0,7),:,:,:].reshape(128,128)
#         img_noise_sim3_2 = img_noise_sim_3[random.randint(0,4),:,:,:].reshape(128,128)
        randidx = random.randint(1,4)
        if randidx==1:
            img_noise_sim1 = img_noise_sim1_1
            img_noise_sim2 = img_noise_sim1_2
        elif randidx==2:
            img_noise_sim1 = img_noise_sim1_1
            img_noise_sim2 = img_noise_sim2_2
        elif randidx==3:
            img_noise_sim1 = img_noise_sim2_1
            img_noise_sim2 = img_noise_sim1_2
        else:
            img_noise_sim1 = img_noise_sim2_1
            img_noise_sim2 = img_noise_sim2_2
...
#         img_noise_sim77 = compute_sim_images(img_noise, patch_size=7, num_select=args.num_sim,
#                                            img_ori=img_noise_norm)
#         img_noise_sim99 = compute_sim_images(img_noise, patch_size=9, num_select=args.num_sim,
#                                            img_ori=img_noise_norm)
        
#         img_noise_sim = img_noise_sim33*0.3+img_noise_sim55*0.5+img_noise_sim77*0.15+img_noise_sim99*0.05
    ...
    pickle.dump(meta_info, open("{}_seg3600_0610gus_ran35_meta_info.pkl".format(args.output_file), "wb"))
